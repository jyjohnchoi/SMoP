...
from .tuners import LoraModel, PrefixEncoder, PromptEmbedding, PromptEncoder, PromptRoutingEmbedding
from .utils import (
    ...
    def get_prompt(self, batch_size):
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            past_key_values = past_key_values.view(
                n_attention_types, # Encoder att, Decoder att, Enc-Dec att
                batch_size,
            ...
            if self.peft_config.num_transformer_submodules == 2:
                past_key_values = torch.cat([past_key_values[0], past_key_values[1], past_key_values[2]], dim=2)
                # print(f"{past_key_values.shape=}") # torch.Size([32, 20, 72, 12, 64])
            else: 
                past_key_values = torch.cat([past_key_values[0], past_key_values[1]], dim=2)

            past_key_values = past_key_values.permute([2, 0, 3, 1, 4]).split(
                # self.peft_config.num_transformer_submodules * 2
                self.peft_config.num_transformer_submodules * n_attention_types
            )
...
class PeftModelForSeq2SeqLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            return self.base_model(input_ids=input_ids, decoder_input_ids=decoder_input_ids, past_prompt=past_prompt, **kwargs)
        else:
            if inputs_embeds is None:
                inputs_embeds = self.word_embeddings(input_ids)
            ...
            if attention_mask is not None:
                # concat prompt attention mask
                try:
                    prefix_attention_mask = kwargs['prefix_attention_mask']
                except:
                    prefix_attention_mask = torch.ones(batch_size, self.peft_config.num_virtual_tokens).to(self.device)
                kwargs["attention_mask"] = torch.cat((prefix_attention_mask, attention_mask), dim=1)
                kwargs.pop('prefix_attention_mask', None)
            # concat prompt labels
            ...
            if self.peft_config.peft_type == PeftType.PROMPT_ROUTING:
                prompts = self.get_prompt_routing(batch_size=batch_size, input_ids=input_ids, inputs_embeds=inputs_embeds, attention_mask=attention_mask)
            else:
                prompts = self.get_prompt(batch_size=batch_size)
            if prompts.shape[1] == batch_size:
                prompts = prompts.transpose(0, 1)
            # print(prompts.shape)
            # print(prompts[:, :self.peft_config.num_virtual_tokens].shape)
    ...
    def generate(self, **kwargs):
        if not isinstance(self.peft_config, PromptLearningConfig):
            ...
            if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
                # past_key_values = self.get_prompt(batch_size)  
                past_prompt = self.get_past_prompt(batch_size)
                kwargs["past_prompt"] = past_prompt
                ...
                inputs_embeds = self.word_embeddings(input_ids)
                if self.peft_config.peft_type == PeftType.PROMPT_ROUTING:
                    prompts = self.get_prompt_routing(batch_size=batch_size, input_ids=input_ids, inputs_embeds=inputs_embeds, attention_mask=kwargs["attention_mask"])
                else:
                    prompts = self.get_prompt(batch_size=batch_size)
                if prompts.shape[1] == batch_size:
                    prompts = prompts.transpose(0, 1)
    ...
    def prepare_inputs_for_generation(self, *args, **kwargs):
        ...
        if model_kwargs["past_key_values"] is None and self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            past_prompt = self.get_past_prompt(batch_size)
            model_kwargs["past_prompt"] = past_prompt
        return model_kwargs

    def get_past_prompt(self, batch_size):
        past_key_values = self.get_prompt(batch_size)  
        prefix_attention_mask = torch.zeros(batch_size, self.peft_config.num_virtual_tokens).to(self.device).bool()
        past_prompt = [{
            "encoder_prompt": {"prev_key": p[0], "prev_value": p[1], "prev_key_padding_mask": prefix_attention_mask}, 
            "decoder_prompt": {"prev_key": p[2], "prev_value": p[3], "prev_key_padding_mask": prefix_attention_mask}, 
            "cross_attention_prompt":{"prev_key": p[4], "prev_value": p[5], "prev_key_padding_mask": prefix_attention_mask}, 

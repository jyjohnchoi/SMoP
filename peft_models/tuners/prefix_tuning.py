...
class PrefixEncoder(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, config):
        super().__init__()
        if config.num_transformer_submodules == 2:
            n_attention_types = 3  # Encoder-Decoder Models
        else: 
            n_attention_types = 2  # Decoder-only Models
        # TODO: add case for encoder-only model
        ...
        if self.prefix_projection and not config.inference_mode:
            ...
            self.transform = torch.nn.Sequential(
                ...
                torch.nn.Tanh(),
                torch.nn.Linear(encoder_hidden_size, num_layers * 2 * token_dim * n_attention_types),
            )
        else:
            self.embedding = torch.nn.Embedding(num_virtual_tokens, num_layers * 2 * token_dim * n_attention_types)
